...
module.exports = class {
  ...
  StatisticsModel() {
    ...
    let time = 0;
    let sobra = 0;
    if (this.party.timeLeftInSec > 256) {
      time = (this.party.timeLeftInSec - sobra) / 256;
      sobra = this.party.timeLeftInSec % 256;
    } else {
      sobra = this.party.timeLeftInSec;
    }
    if (sobra > 128) {
      sobra = sobra - 256;
    }
